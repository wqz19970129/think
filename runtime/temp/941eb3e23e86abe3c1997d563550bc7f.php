<?php if (!defined('THINK_PATH')) exit(); /*a:3:{s:70:"E:\www\think\public/../application/admin/view/default/article\add.html";i:1496373782;s:70:"E:\www\think\public/../application/admin/view/default/public\base.html";i:1496373782;s:75:"E:\www\think\public/../application/admin/view/default/article\sidemenu.html";i:1496373782;}*/ ?>
<!doctype html>
<html>
<head>
    <meta charset="UTF-8">
...
<script>
	...
	<div class="main-title cf">
		<h2>
			...
			<?php if(count($rightNav) > $i): ?><i class="ca"></i><?php endif; endforeach; endif; else: echo "" ;endif; if(isset($article)): ?>：<a href="<?php echo url('article/index','cate_id='.$info['category_id'].'&pid='.$article['id']); ?>"><?php echo $article['title']; ?></a><?php endif; ?>
			]
		</h2>
	</div>
	<!-- 标签页导航 -->
<div class="tab-wrap">
	...
	<form id="form" action="<?php echo url('update'); ?>" method="post" class="form-horizontal">
        ...
        <div id="tab<?php echo $key; ?>" class="tab-pane <?php if($key == '1'): ?>in<?php endif; ?> tab<?php echo $key; ?>">
            <?php if(is_array($fields[$key]) || $fields[$key] instanceof \think\Collection || $fields[$key] instanceof \think\Paginator): $i = 0; $__LIST__ = $fields[$key];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$field): $mod = ($i % 2 );++$i;if($field['is_show'] == 1 || $field['is_show'] == 2): ?>
                <div class="form-item cf">
                    ...
                    <div class="controls">
                        <?php switch($field['type']): case "num": ?>
                                <input type="text" class="text input-mid" name="<?php echo $field['name']; ?>" value="<?php echo $field['value']; ?>">
                            <?php break; case "string": ?>
                                <input type="text" class="text input-large" name="<?php echo $field['name']; ?>" value="<?php echo $field['value']; ?>">
                            <?php break; case "textarea": ?>
                                <label class="textarea input-large">
                                <textarea name="<?php echo $field['name']; ?>"><?php echo $field['value']; ?></textarea>
                                </label>
                            <?php break; case "date": ?>
                                <input type="text" name="<?php echo $field['name']; ?>" class="text date" value="" placeholder="请选择日期" />
                            <?php break; case "datetime": ?>
                                <input type="text" name="<?php echo $field['name']; ?>" class="text time" value="" placeholder="请选择时间" />
                            <?php break; case "bool": ?>
                                <select name="<?php echo $field['name']; ?>">
                                    <?php $_result=parse_field_attr($field['extra']);if(is_array($_result) || $_result instanceof \think\Collection || $_result instanceof \think\Paginator): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
                                        <option value="<?php echo $key; ?>" <?php if($field['value'] == $key): ?>selected<?php endif; ?>><?php echo $vo; ?></option>
                                    <?php endforeach; endif; else: echo "" ;endif; ?>
                                </select>
                    			...
                    			<div class="controls">
                    				...
                    				<div class="upload-img-box">
                    					<?php if(isset($data[$field['name']])): ?>
                    						<div class="upload-pre-file"><span class="upload_icon_all"></span><?php echo $data[$field['name']]; ?></div>
                    					<?php endif; ?>
                    				</div>
                    			</div>
                    			<script type="text/javascript">
		...
		<div class="form-item cf">
			...
			<a class="btn btn-return" href="<?php echo url('article/index?cate_id='.$cate_id); ?>">返 回</a>
			<?php if(config('open_draftbox') and (request()->action() == 'add' or $info['status'] == 3)): ?>
...
$(function(){
	...
	<?php if(config('open_draftbox') and (ACTION_NAME == 'add' or $info['status'] == 3)): ?>
	//保存草稿
	var interval;
